...
async function tagEveryone(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
			...
			let noEveryFormatted = groupMembersIdFormated;
            let noEveryone = groupMembersId;

            if (functions.dnd && Array.isArray(functions.dnd.values)) {
                noEveryFormatted = groupMembersIdFormated.filter(
                    (usr) => !functions.dnd.values.some((ir) => ir.replace(/@s.whatsapp.net/gi, '') === usr)
                );
                noEveryone = groupMembersId.filter(
                    (usr) => !functions.dnd.values.includes(usr)
                );
            }
