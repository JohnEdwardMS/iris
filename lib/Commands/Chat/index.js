...
let gtts, cleverbot, simsimi, gpt4all, transformers;

try {
    gtts = require('@killovsky/gtts');
} catch (err) {
    console.warn('Módulo gtts não disponível.');
}

try {
    cleverbot = require('cleverbot-free');
} catch (err) {
    console.warn('Módulo cleverbot-free não disponível.');
}

try {
    simsimi = require('chats-simsimi');
} catch (err) {
    console.warn('Módulo chats-simsimi não disponível.');
}

try {
    gpt4all = require('../../Scripts/IAs/Chatbots/gpt4all');
    transformers = require('../../Scripts/IAs/Chatbots/gpt4all');
} catch (err) {
    console.warn('Módulos GPT (gpt4all ou transformers) não disponíveis.');
}
/* ^JOHN MODIFICATION^ */
